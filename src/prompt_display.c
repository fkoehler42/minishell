...
/*   Updated: 2016/05/21 15:17:32 by fkoehler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char	*get_relative_path_prompt(t_env *home, t_env *pwd, t_env *oldpwd)
{
	int		i;
	char	*prompt;
...
char		*get_prompt(t_env *env_lst)
{
	...
	if (!pwd || !pwd->value[0])
		...
		g_prompt = get_relative_path_prompt(home, pwd, oldpwd);
	return (g_prompt);
}
